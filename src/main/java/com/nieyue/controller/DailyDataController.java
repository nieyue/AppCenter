...
public class DailyDataController {
	...
	public @ResponseBody StateResultList browsePagingDailyData(
			...
			list= dailyDataService.browsePagingDailyData(createDate,appId,acountId,pageNum, pageSize, orderName, orderWay);
			if(list.size()>0){
	...
	public @ResponseBody StateResultList statisticsDailyData(
			...
			@RequestParam(value="orderWay",required=false,defaultValue="desc") String orderWay,HttpSession session)  {
		List<DailyData> l = dailyDataService.statisticsDailyData(startDate,endDate,appId,acountId,pageNum, pageSize, orderName, orderWay);
		if(l.size()>0){
	...
	public @ResponseBody int countAll(
			...
			@RequestParam(value="acountId",required=false)Integer acountId,
			HttpSession session)  {
		int count = dailyDataService.countAll(createDate,appId,acountId);
		return count;
	}
	/**
	 * 每日数据单个加载
	...
	public @ResponseBody StateResultList loadDailyDataByParams(
			...
			HttpSession session)  {
		List<DailyData> list = new ArrayList<DailyData>();
		DailyData dailyData = dailyDataService.loadDailyData(dailyDataId,crateDate,appId,acountId);
		if(dailyData!=null &&!dailyData.equals("")){
			list.add(dailyData);
			return ResultUtil.getSlefSRSuccessList(list);
		}else{
